...
def fit_hmm(
    ...
    Inputs are raw numpy/tuples so Streamlit can hash and cache them.
...
def transition_matrix(transmat: np.ndarray, label_map: dict, n_regimes: int) -> pd.DataFrame:
    ...
    trans = pd.DataFrame(
        transmat[np.ix_(states_ordered, states_ordered)],
